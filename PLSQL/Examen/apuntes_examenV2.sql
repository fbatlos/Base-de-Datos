--PL SQL
    ...
    --bloque anonimo 
        declare 
            ...
            vn_numero number(2) := 0
            --declarar sin asignar
            vv_texto varchar2(3)
        begin
        ...
        exception
            ...
            --todo bloque tiene que acabar en end
    
    --bloque definido
        create or replace PR_nombre is
            --igual que el declare
        begin
            --todo lo que quieras
        exception
            --sona de errores
    ...
    --bloque anidados
        --se puede anidar tanto en la zona declarativa como en la ejecutiva
        declare 
            --sentencias declarativa
            --declarar una variable
            vn_numero number(2) := 0
    ...
    vn_numero := 0
    vv_texto := 'hola'
    --definir una constante en el declare
    cn_numero constant numero := 1
...
--cursores
	...
	--explicitos dinamicos
	declare 
		cursor c_nombre(numero number) is
			select *
			from tabla
	...
	-- abrir un cursor con fetch
	declare 
		variableCursor C_apellidoYfecha%ROWTYPE; 
		cursor c_nombre(numero number) is
			select *
			from tabla
			where columna = numero;
	begin 
		...
		open nombreCursor
		loop
            --esto es como un for raro
			FETCH c_nombre INTO variableCursor;
            --esto es is no encuentra linea
...
--excepciones
    ...
    --estructura
        declare
        --declaramos una variable excepcion
        vn_numero number(2):= 1
        ex_nombre exception;
        ...
        begin
        ....
        --decimos que si pasa una cosa tiro la excepcion
        if vn_numero = 1 then
            raise ex_nombre
        end if;
        exception
        ...
        when others then
            sentencias;
            --para excepciones no controladas tenemos
...
--eliminar procedimiento
    drop procedure pr_nombre

--crear procedimiento con parametros
-- parametro de entrada es "in" permite pasara un valor a subprograma y puede ser una variable o constante
-- parametro de salida "out" permite devolver el valor que se ha procesado, solo puede ser un a variable
-- el parametro in out es para pasarle un valor inicial, y regresar ese avlor ya procesaod
    create procedure pr_nombre(
                                pi_n_param in number,
                                nombre_parametro elegir si es una de estas opciones(in-out-in out) in tipodato
                                po_v_param2 out varchar2
                                pio_d_param3 in out date
                                ) is 
        --seccion declarativa
    begin 
        -- seccion ejecutiva
...
--invocar procedimiento
    ...
    begin 
        --seccion ejecutiva
        PR_nombre(parametros)
    end;
    --tambien puedes con 
    exec PR_nombre(parametros)

--crear funcion 
    create function F_nombre
    return tipodedatoaretornar is
        --seccion declarativa
    begin
        --seccion ejecutiva 
        return valor
...
--modificar funcion o procedimiento
...
--invocar una funcion 
    ...
    begin 
        --seccion ejecutiva
        vn_resultado = F_nombre(parametros)
    end;

-- paquetes en base de datos 
    -- te permite agrupar funciones o procedimientos en una "libreria"

    --creacion de un paquete
    --cambias la parte "publica del paquete"
        create or replace package pk_nombre is 
        --funciones o procedimientos
        end pk_nombre;

    --creacion del body del paquete
    --esta es la parte interna del paquete
        create or replace package body pk_nombre is
        --todo el codigo de las funciones o procedimiento
        end pk_nombre;

    --invocar un paquete
    --creacion del paquete 
        CREATE PACKAGE pk_nombre IS
        FUNCTION fn_A
        RETURN NUMBER IS
        BEGIN
        RETURN 1
    ...
    --invocacion
        PROCEDURE pr_B IS
        BEGIN
        SELECT pk_nombre.fn_A --elegimos el paqiuete, funcion o procedimiento del mismo
        INTO x 
        FROM DUAL; -- Corregido
        EXCEPTION 
        ...
    ...
    --borra un paquete cuerpo
        drop package body pk_nombre
        --eliminas el codigo interno
    -- borra un paquete
        --borras todo el paquete cuerpo y especificacion
        drop package pk_nombre

--COMANDOS BASICOS:
                comando: descripcion
                ------------------------------------------------------------------------------------------------
                START ARCHIVO.SQL: Arranca un fichero de comandos almacenado previamente
                ------------------------------------------------------------------------------------------------
                @filename: Igual que START
                ------------------------------------------------------------------------------------------------
                EXIT: Abandonar SQL*Plus
                ------------------------------------------------------------------------------------------------
                CLEAR SCR: Limpiar la pantalla
                ------------------------------------------------------------------------------------------------
                SPOOL y SPOOL OFF: Sirve para almacenar todo lo que hacemos en ficheros
                ------------------------------------------------------------------------------------------------
                DESC: Lista los atributos de las tablas
                ------------------------------------------------------------------------------------------------
                SET PAUSE ON: Hace que SQL*Plus pare al principio de cada página. Sólo se 
                pasa a la siguiente página cuando el usuario pulsa RETURN.
                ------------------------------------------------------------------------------------------------
                ...
                ------------------------------------------------------------------------------------------------
                COLUMN columna FORMAT a50: Darle más longitud a una columna (no numérica)
                ------------------------------------------------------------------------------------------------
                SHOW USER: Muestra el usuario que está conectado


--Tipos de objetos de una base de datos
                Objeto                Descripción
                ------------------------------------------------------------------------------------------------
                Tabla                 Unidad básica de almacenamiento, formada por filas y columnas
                ------------------------------------------------------------------------------------------------
                Vista                 Representación lógica de un subconjunto de datos extraídos de una o más tablas
                ------------------------------------------------------------------------------------------------
                Secuencia         Estructura que genera valores automáticos
                ------------------------------------------------------------------------------------------------
                Índices             Variables que mejoran la eficiencia de consultas a las bases de datos
                ------------------------------------------------------------------------------------------------
                Sinónimos         Estructuras que dan nombres alternativos a otros objetos
                ------------------------------------------------------------------------------------------------
                ◦ Los nombres de tablas y de las columnas deben empezar con una letra y no deben superar los 30 caracteres 
                ◦ Deben contener solo los caracteres A-Z, a-z, 0-9, _, $, #
                ◦ Los nombres no deben duplicarse para el mismo usuario, 
                pero si pueden ser igual para distintos (Concepto de esquema)
                ◦ No deben ser palabras reservadas de ORACLE.
                ◦ Deben usarse nombres descriptivos para cualquier objeto de la base de datos
                ◦ En el caso de tener campos relacionados, se debe intentar poner el mismo nombre para los dos.
...
--TIPOS DE DATOS EN LA TABLA 
                ...
                ------------------------------------------------------------------------------------------------
                VARCHAR2(n):                Almacena datos alfanuméricos de longitud variable. 
                                                        Si el valor insertado es menor que N, el resto lo ajusta al tamaño del valor  Ganamos espacio de almacenamiento
                ------------------------------------------------------------------------------------------------
                CHAR(n):                        Almacena datos alfanuméricos de longitud fija. 
                                                        Si el valor insertado es menor que N, 
                                                        el resto lo rellena con espacios en blanco.
                ------------------------------------------------------------------------------------------------
                NCHAR NVARCHAR2:        Almacena datos alfanuméricos de 
                                                        longitud fija-variable usando caracteres Unicode.
                ------------------------------------------------------------------------------------------------
                NUMBER(p, s):             Almacena números. La p representa los 
                                                        dígitos totales, y la s los dígitos decimales.
                ------------------------------------------------------------------------------------------------
                DATE:                             Almacena fechas y horas concretas.
                ------------------------------------------------------------------------------------------------
                TIMESTAMP:                     Más específico que Date, almacenando incluso fracciones de segundo.
                ------------------------------------------------------------------------------------------------
                TIMESTAMP WITH TIME ZONE:                Almacena fechas relativas a la zona horaria y a la región
                ------------------------------------------------------------------------------------------------
...
--SENTENCIAS
                ...
                ------------------------------------------------------------------------------------------------
                DEFAULT:                si el usuario no introduce nada en ese campo, se le designa el texto que este dentro
                                                ejemplo: campo sexo, so el ususario a crear la nueva linea no introduce nada que sea H de hombre
                                                SEXO char(1) DEFAULT 'H'
                ------------------------------------------------------------------------------------------------
                NOT NULL:    NO PUEDE SER NULO
                ------------------------------------------------------------------------------------------------
                UNIQUE: DEBE DE SER UNICO
                ------------------------------------------------------------------------------------------------
                PRIMARY KEY: LA CLAVE PRINCIPAL
                ------------------------------------------------------------------------------------------------
                FOREIGN KEY: CLAVE FOREANEA O HEREDADA
                ------------------------------------------------------------------------------------------------
                CHECK: TE COMPRUEBA SI EL VALOR DE LA COLUMNA CUMPLE LA CONDICION

--COMENTARIOS
-- comentar a nivel de tabla
                COMMENT ON (TABLE TABLA)
                IS 'COMENTARIO'
                -- comentar a nivel de columnas
                COMMENT ON (COLUMN TABLA.COLUMNA)
                IS 'COMENTARIO'
                -- PARA BORRAR UN COMENTARIO PON ESTA MISMA SENTENCIA PERO CON EL IS VACIO
                -- PARA ACCEDER A LOS COMENTARIOS SON ESTAS TABLAS
                ALL_COL_COMMENTS
                ◦ USER_COL_COMMENTS
                ◦ ALL_TAB_COMMENTS
                ◦ USER_TAB_COMMENTS
...
-- CONSTRAINT
        ...
        USER_CONSTRAINTS
                --SENTENCIAS
                        ...
                        ------------------------------------------------------------------------------------------------
                        UNIQUE                    La/s columna/s deben tener valores únicos
                        ------------------------------------------------------------------------------------------------
                        PRIMARY KEY         Definimos la clave primaria de una tabla
                        ------------------------------------------------------------------------------------------------
                        FOREIGN KEY         Relaciona datos de diferentes tablas mediante integridad referencial
                        ------------------------------------------------------------------------------------------------
                        CHECK                     Una determina condición que debe cumplirse para una columna
                        ------------------------------------------------------------------------------------------------
                --Crear constraints
                        --definir a nivel de columnas 
                        CREATE TABLE TABLA
                        (CAMPO1 TIPO_DATO CONSTRAINT MENSAJE TIPO_CONSTRAINT,
                        ...)
                        ejemplo:
                        create table tienda
                        (nif char(9) CONSTRAINT PK_nif PRIMARY KEY 
                        nombre varchar2(10));


                        --definir a nivel de tabla
                        CREATE TABLE nombre_tabla
                        (COLUMNAPRINCIPAL TIPO_DATO ,
                        COLUMNA TIPO_DATO DEFAULT 'XD'
                        ..., CONSTRAINT NOMBRE_CONSTRAINT TIPO_RESTRICCION (COLUMNAPRINCIPAL), ...)
                        ejemplo:
                        create table tienda
                        (nif char(9),
                ...
                create table tienda
                        (nif char(9),
                        nombre char(4),
                        CONSTRAINT PK_nif PRIMARY KEY(nif,nombre));
                ...
                create table camion
                        (matricula char(2),
                        nif char(9),
                        CONSTRAINT FK_KEY foreign key (nif) references TIENDA(nif) ON DELETE SET NULL);
                ON DELETE CASCADE: Cuando se elimina una fila referenciada, todas las filas en la tabla hija que hacen referencia a esa fila también se eliminan.
                ON DELETE SET NULL: Cuando se elimina una fila referenciada, los valores de las columnas de clave foránea en las filas de la tabla hija que hacen referencia a esa fila se establecen en NULL.
                ON DELETE NO ACTION: No se permite eliminar una fila referenciada. Si se intenta, se produce un error.
                check: permite    comoprobar si cumple la condicion

                        create table coche
                        (matricula char(9),
                        color char(9),
                        CONSTRAINT camion_color_ck CHECK (color ='negro' OR color ='verde' OR color ='azul' OR color ='dorado'));

--MODIFICAR TABLAS
                --BORRAR TODAS LAS FILAS TABLAS,PERO MANTENER SU ESTRUCTURA
                                TRUNCATE TABLE TABLA
                --BORRAR TABLA ENTERA
                                DROP TABLE TABLA
                                --BORRAR ALGUNAS FILAS
                                DROP TABLE TABLA
                                WHERE CONDICIONES DE LAS FILAS A CUMPLIR
                --RENOMBRAR TABLAS
                RENAME TABLA TO NUEVO_NOMBRE_TABLA
                --AÑADIR NUEVAS COLUMNAS 
                                -- añadir una nueva columna
                                        ALTER TABLE TABLAAMODIFICAR
                                        ADD COLUMN NUEVA_COLUMNA TIPODEDATO SENTENCIAS;
                                --añadir varias columnas
                                        ALTER TABLE TABLAAMODIFICAR
                                        ADD (PRIMERA_NUEVA_COLUMNA TIPODEDATO SENTENCIAS,
                                                SEGUNDA_NUEVA_COLUMNA TIPODEDATO SENTENCIAS,
                                                ...);
                --MODIFICAR COLUMNAS
                                ALTER TABLE TABLAAMODIFICAR
                                MODIFY COLUMNA_EXISTENTE NUEVO_TIPODEDATO NUEVAS_SENTENCIAS
                                ...;
                --ELIMINAR COLUMNAS
                                ALTER TABLE TABLAAMODIFICAR
                                DROP COLUMN COLUMNAAELIMINAR,
                                ...;


--INSERT
--sentencia para añadir filas
                insert into TABLA(col1,col2,...)
                values(valor_para_col1,valor_para_col2,'valor')
                select 'valor'
                from TABLA
                where condicion
--UPDATE
--Sentencia para modificar los datos de la fila de una tabla.
                UPDATE TABLA
                SET COLUMNA A CAMBIAR = (CONSTANTE O SELECT COLUMNA FROM TABLA)
                WHERE COLUMNA A CAMBIAR > (CONSTANTE O SELECT COLUMNA FROM TABLA);
...
Abortar transacciones	ROLLBACK;


--sub consultas
--TU PADRE VA A RESUMIR ESTO :D
...
-- Se pueden construir CONDICIOES MULTIPLES utilizando los operadores logicos Booleanos estandares (OR, AND y NOT)
...
-- PRUEBA NICO
SELECT TRUNC (MONTHS_BETWEEN (SYSDATE, FECHA_ALT)/12)
    FROM EMPLE;
-- PRUEBA NICO
