--Fco José Batista de los Santos. Nota final : 7,43
...
WHERE E.Dpto_em = D.Cod_de                              ---Miara esto
...
from empleados
WHERE (SALARIO_EM+Comision_em)> 1300           --- con el nvl hay que mirarlo
ORDER BY cod_em ASC;
...
from empleados
GROUP BY DPTO_EM;
--falta NVL , es primordial cuando no hay datos.
...
SELECT Dpto_em,AVG(SALARIO_EM)  ---**Otra vez el group by , el where en el having
FROM EMPLEADOS
GROUP BY Dpto_em
Having  max(Salario_em)< (SELECT AVG(Salario_em)
                        FROM empleados
                       );
